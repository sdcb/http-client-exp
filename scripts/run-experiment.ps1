...
Add-Content -Path $summaryLog -Value ("[{0}] Build start" -f (Get-Date -Format o))
& dotnet build (Join-Path $root "exp.sln") -c Debug | Tee-Object -FilePath (Join-Path $logDir "build.log") | Out-Host
Add-Content -Path $summaryLog -Value ("[{0}] Build complete" -f (Get-Date -Format o))

$env:HTTPLEAK_URL = "http://localhost:5055"
$serverProject = Join-Path $root "Server\HttpLeakServer\HttpLeakServer.csproj"
$serverProc = Start-Process -FilePath "dotnet" -ArgumentList @("run", "--project", $serverProject, "--no-build") -WorkingDirectory $root -RedirectStandardOutput $serverLog -RedirectStandardError $serverErr -PassThru
$serverProc.Id | Set-Content -Path $serverPidFile

$serverReady = $false
for ($i = 0; $i -lt 30; $i++) {
    try {
        $response = Invoke-WebRequest -Uri "http://localhost:5055/ping" -TimeoutSec 2 -UseBasicParsing
...
if (-not $serverReady) {
    throw "Server did not start within timeout. Check logs: $serverErr"
}

$baseline = (Get-ConnCounts).TimeWait
Write-ConnSample "before-net48"
$net48Exe = Join-Path $root "Clients\HttpLeakClient.Net48\bin\Debug\net48\HttpLeakClient.Net48.exe"
...
Add-Content -Path $summaryLog -Value ("[{0}] Experiment complete" -f (Get-Date -Format o))
Write-Host "All done. Logs at: $logDir"
